...
# ------------------- Agrégation Q-values -------------------
def aggregate_qvalues(qtab, tc, states):
    """
    Moyenne pondérée des Q-values selon les visites,
    avec correction EV=-inf si action illégale.
    """
    q_sum = np.zeros(4, dtype=float)
    total = 0
    for s in states:
        v = qtab.visits_by_tc[tc][s].sum()
        q_sum += qtab.q_by_tc[tc][s] * max(1, v)
        total += max(1, v)
    ...
    for total in range(5, 22):
        ...
        for dealer_card in range(2, 12):
            ...
            if show_ev:
                ...
                # pas de P_EV car -inf en hard
        rows.append(row)
    return pd.DataFrame(rows)


# ------------------- Soft totals -------------------
def build_table_soft(qtab, env, tc, show_ev=True):
    rows = []
    for val in range(2, 10):  # A2 .. A9
        ...
        for dealer_card in range(2, 12):
            ...
            if show_ev:
                ...
                row[f"{dealer_card} S_EV"] = f"{qvals[1]:.3f}"
                row[f"{dealer_card} D_EV"] = f"{qvals[2]:.3f}"
                # pas de P_EV car -inf en soft
        rows.append(row)
    return pd.DataFrame(rows)


# ------------------- Pairs -------------------
def build_table_pairs(qtab, env, tc, show_ev=True):
    rows = []
